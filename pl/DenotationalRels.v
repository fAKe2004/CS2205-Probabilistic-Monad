...
Proof.
  intros.
  Sets_unfold in H. 
  (* Sets_unfold 会保留集合语言，sets_unfold 展开到映射定义 *)
  Sets_unfold.
  intros a b.
  specialize (H a b).
  tauto.
...
Goal forall (A: Type) (X Y Z: A -> A -> Prop),
  X ∘ (Y ∘ Z) == (X ∘ Y) ∘ Z.
Proof.
  intros.
  Sets_unfold.
...
(* ∩ ∪ etc. 均保持集合相等关系 *)
Goal forall (A: Type) (X1 X2 Y: A -> Prop),
  X1 == X2 ->
  X1 ∪ Y ∩ X2 == X2 ∪ Y ∩ X1.
Proof.
  intros.
...
Theorem if_seq: forall e c1 c2 c3,
  [[ if (e) then {c1} else {c2}; c3 ]] ~=~ [[ if (e) then {c1; c3} else {c2; c3} ]].
Proof.
  intros.
  unfold cequiv.
  simpl.
  unfold while_sem, if_sem, seq_sem.
