...
Proof.
  ...
  + transitivity x0; [| apply reflexivity_setoid; tauto]. 
    (* 利用 x0 展开 y <= x0 x0 <= y0, 对第二部分用 reflexivity.  *)
...
Proof.
  ...
  (* a1 == a2 <=> a1 <= a2 /\ a2 <= a1 (偏序性质 antisymmetricity) *)
        ...
        {oLubA: OmegaLub A} {BotA: Bot A}: Prop :=
{
  ...
  (* completeness 要求显示存在 omega_lub 这样一个计算 lub 的函数，并要求在链上 omega_lub 的正确性 *)
  bot_is_least: is_least bot
  (* bot_is_least 要求显示指出最小元 bot 正确*)
}.
...
Definition continuous
  ...
  forall l: nat -> A,
    ...
    (* 这里 assume: fun n => f (l n) 生成的也是一条链，其实假设了 mono f *)
...
Proof.
  ...
  (* 单调 -> omega_lub 算出的确实是上界 *)
  unfold is_omega_ub.
  clear H2 H3. (* remove unneeded props. *)
  induction n; simpl.
  + apply bot_is_least.
  + rewrite <- H1.
    apply H. (* mono f , f x <= f y <== x <= y *)
    apply IHn.
Qed.
...
(* OK, 开始运用 Kleene fixpoint theorem 给出 WHILE 定义 *)
...
(* fix Y, then function: X -> Y ∘ X is monotonic *)
...
(* fix Y, then function: X -> Y ∘ X is continuous *)
...
(* fix Y, then function X -> X ∪ Y is mono *)
...
(* fix Y, then function X -> X ∪ Y is continuous *)
